...
export const reqProductSearch = (pageNum, pageSize, selectKey, inputValue) => {
    if (selectKey === '1') {
        // 根据名称获取商品列表
        return ajax('/manage/product/search', { pageNum, pageSize, productName: inputValue })
    } else {
        // 根据描述获取商品列表
        return ajax('/manage/product/search', { pageNum, pageSize, productDesc: inputValue })
    }
}
// 根据分类ID获取分类
export const reqCategoryById = (categoryId) => ajax('/manage/category/info', { categoryId })
// 对商品进行上架/下架处理
export const reqChangeSale = (data) => ajax('/manage/product/updateStatus', data, 'post')
// 删除图片的接口
export const reqDeleteImg = (data) => ajax('/manage/img/delete', data, 'post')
// 添加或更新商品
export const reqAddOrUpdate = (data, isUpdate) => ajax(`/manage/product/${isUpdate ? 'update' : 'add'}`, data, 'post')
// 获取角色列表接口
export const reqRoleList = () => ajax('/manage/role/list')
// 添加角色接口
export const reqAddRole = (data) => ajax('/manage/role/add', data, 'post')
// 设置角色权限接口
export const reqSetRoleAuth = (data) => ajax('/manage/role/update', data, 'post')
// 获取用户列表接口
export const reqUserList = () => ajax('/manage/user/list')
// 删除用户接口
export const reqDeleUser = (data) => ajax('/manage/user/delete', data, 'post')
// 添加用户接口
export const reqAddOrUpadateUser = (data) => ajax(`/manage/user/${data._id ? 'update' : 'add'}`, data, 'post') 